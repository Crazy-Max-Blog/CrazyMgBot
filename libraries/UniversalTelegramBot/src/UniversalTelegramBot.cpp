...
int UniversalTelegramBot::getUpdates(long offset) {
  ...
  if (response == "") {
    ...
    if (!error) {
	 ...
	 String chat_id = doc["result"][0]["message"]["from"]["id"];
	   Serial.println(chat_id);
	   String text = "Слишком длинное сообщение!";
	   return(sendSimpleMessage(chat_id, text, "Markdown"));
        #ifdef TELEGRAM_DEBUG  
            Serial.println(F("Уфф, пропускаем сообщение, слишком длинное"));
        #endif	 
...
String UniversalTelegramBot::sendPostDocument(JsonObject payload) {
  ...
  String response = "";
  #ifdef TELEGRAM_DEBUG  
    Serial.println(F("sendPostDocument: SEND Post Document"));
  #endif
  unsigned long sttime = millis();
